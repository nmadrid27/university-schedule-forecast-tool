...
- **Resolved (July 11, 2026 review; was the May 5 open issue)**: campus expansion from `Savannah` to `Savannah | SCADnow` on First-Year seq-map rows produces only ~10 students of NOW demand (FOUN 230 NOW = 10.5 vs ACT 37; FOUN 240 NOW = 10.5 vs ACT 64) because of equal-share `source_totals` dilution (144 Winter FOUN 112 NOW students x 0.95 x 1/13 programs x 1.10 buffer = 11.6), compounded for co-requisite rows by a verified engine bug: non-CHOICE concurrent targets ("FOUN 240; FOUN 245") each inflate `closer_source_totals` by 1.0, so co-req demand is split 1/k instead of duplicated (`load_sequence_mappings`, source_totals accumulation around `api/forecaster.py:400`). It is NOT a campus-tag/CRN-lookup disconnect; the dilution is already per-campus.
