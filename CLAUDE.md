...
- **Resolved (July 11, 2026 review; was the May 5 open issue)**: campus expansion from `Savannah` to `Savannah | SCADnow` on First-Year seq-map rows produces only ~10 students of NOW demand (FOUN 230 NOW = 10.5 vs ACT 37; FOUN 240 NOW = 10.5 vs ACT 64) because of equal-share `source_totals` dilution (144 Winter FOUN 112 NOW students x 0.95 x 1/13 programs x 1.10 buffer = 11.6), compounded for co-requisite rows by a verified engine bug, FIXED 2026-07-11 (commit `8dee3e1`): non-CHOICE concurrent targets ("FOUN 240; FOUN 245") each inflated `closer_source_totals` by 1.0, so co-req demand was split 1/k instead of duplicated. Each program-row now contributes its weight once per source course in `load_sequence_mappings`. It is NOT a campus-tag/CRN-lookup disconnect; the dilution is already per-campus. Backtest impact: SAV capture 81.7% to 87.2%, combined 74.6% to 79.3% (see `docs/SPRING_2026_BACKTEST.md`, "Post co-requisite fix").
...
- Backend suite green: **567 passing, 0 failing** (frontend 145). The former 2 ratio-fallback failures were a real production bug, fixed 2026-06-06: a redundant local `resolve_term_info` import in `run_forecast` caused `UnboundLocalError` on the Summer ratio-fallback path (swallowed into a 500, so every Summer forecast failed).
- 2026-07-11 review/fix pass (branch `fix/review-2026-07-11`): four verified bugs fixed test-first: co-requisite target demand split 1/k in `load_sequence_mappings` (see Campus Detection bullet); output `set` adjustments never injected rows when the base forecast was empty (the `and rows` guard in `run_forecast` defeated the 422's own manual-estimate advice); `cvMape` on the ensemble endpoint reported RMSE (optimizer now runs with `metric="mape"`); `/api/data/import` now validates uploads with the real loader, backs up the previous file to `.bak`, and lands via atomic `os.replace` (a wrong or corrupt upload can no longer destroy the live Master Schedule). A second review round (pre-push, same day) fixed: `/api/backtest` now scores the raw model (buffer forced to 0 unless explicitly set, manual adjustments skipped via `ForecastRequest.applyAdjustments`, demand metric resolved once for both the forecast and actuals sides); per-course ensemble weight/cvMape state (optimized weights no longer leak into unoptimizable courses); tolerant `method`/`demandMetric` request strings (normalized in handlers, not rejected by Pydantic); partial `PUT /api/config` no longer clobbers `method`/`demand_metric`; import guards for `.xls` and CSV admits with actionable messages plus a real PZSAAPF header check; the CSV loader mirrors the xlsx CRN-dedupe ordering (blank-CRS continuation rows no longer suppress data rows) and blank `MAX ENR` cells fall back to ACT; injected adjustment rows normalize campus casing to display labels; `/api/terms` offers historical terms when ANY post-rollout prior is imported; the frontend term dropdown falls back to static options when `/api/terms` returns empty lists. A third round (2026-07-12, deferred-findings sprint) fixed the rest: calendar-year OLS x-axis (gap years count; `forecast_next_season`/LOO anomaly and `run_sameseason_forecast` extrapolate to the actual target year); `farther_source_totals` accumulates from unfiltered farther courses, mirroring the closer side; `load_all_term_enrollments` parses the Master Schedule once for the anomaly history (was one full parse per term, 2.65s to 0.14s on the 18-term CSV) with one shared row processor for CSV and xlsx; the ensemble fits each model on the same-season sub-series for the season after the last observation and labels results with `season` (mixed-series fallback only below 2 same-season points); `{term}` path segments are validated (400 on metacharacters); `settings.json` is written 0600; raw LLM SDK errors are logged, not surfaced; one `CAMPUS_DISPLAY` constant, shared admits row decoder, shared ratio-fallback predicate, backtest metrics reuse temporal-CV math, auto fallback always attributed in the method label; frontend request building and API types deduplicated. Still open (deliberate): FIBERS "FOUN 220; FOUN 240" co-req-vs-choice (domain call; mark the cell `CHOICE:` if students pick one), 2-point OLS extrapolates at full slope (modeling choice), scenario sections ignore scoped capacity overrides, `detect_anomaly` cannot flag a jump off a zero trend, Atlanta admits are skipped (`_ADMIT_CAMPUS_MAP` maps M/O only; Atlanta scope is an open product question), the ratio fallback always uses ACT ENR regardless of demand metric, and adjustments JSON writes are not atomic (single-user risk).
